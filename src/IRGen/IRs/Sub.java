...
public class Sub extends IR{
    private Var to;
    private Var from1;
    private Var from2;

    public Sub(Var to, Var from1, Var from2) {
        this.to = to;
        this.from1 = from1;
        this.from2 = from2;
        super.setIrType(IRType.SUB);
    }
    ...
    public String toAsm() {
        ...
        if (toC.getVarType() == VarType.REG) {
            sb.append(String.format("subu %s, %s, %s\n", toC.getName(), from1C.getName(), from2C.getName()));
        } else {
            sb.append(String.format("subu %s, %s, %s\n", Var.tmpReg1(), from1C.getName(), from2C.getName()));
